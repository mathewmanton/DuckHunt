...
:	mhAppInst(hInstance),
	...
	mClientWidth(GetDeviceCaps(GetDC(NULL), HORZRES)),
	mClientHeight(GetDeviceCaps(GetDC(NULL),VERTRES)),
	mEnable4xMsaa(false),
	mhMainWnd(0),
	mAppPaused(false),
	mMinimized(false),
...
float D3DApp::getMClientHeight() const
{
	return (float)mClientHeight;
}

float D3DApp::getMClientWidth() const
{
	return (float)mClientHeight;
}
...
void D3DApp::ShowCursors(bool show)
{
	ShowCursor(show);
}
